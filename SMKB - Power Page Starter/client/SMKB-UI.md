...
> Library: `@smkb/design-ui` (tokens + components)
...
```bash
pnpm add @smkb/design-ui
```

```ts
// main.ts — import order matters
import '@smkb/design-ui/tokens.css'       // light theme tokens + custom fonts (Open Sans, Foodi, Assistant)
import '@smkb/design-ui/tokens-dark.css'  // dark theme overrides
import '@smkb/design-ui/styles'           // component styles + base CSS
...
```ts
// main.ts (Power Apps)
import '@smkb/design-ui/tokens-nofonts.css'  // ← no @font-face, system fonts only
...
```ts
import { SmkbButton, SmkbInput, SmkbSelect } from '@smkb/design-ui'
```
...
Circular icon-only button. Renders an icon from **`@smkb/design-ui` SVG files** (`icon` prop), or custom content via the default slot.
...
Renders one SVG from **`src/icons/svg/*.svg`** in `@smkb/design-ui` (Material Symbols–style glyphs). Export `SMKB_ICON_NAMES` lists ids.
