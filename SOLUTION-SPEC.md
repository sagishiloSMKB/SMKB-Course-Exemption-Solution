...
| | |
|---|---|
| **מה הפתרון עושה** | מאפשר למועמד/ת לתוכנית לימודים ב-SMKB לדווח על לימודים קודמים ותעודות מקצועיות, מציע לבודק/ת אנושי/ת התאמות אפשריות בין הלימודים הקודמים לדרישות התוכנית, ומפיק אישור פטור **מייעץ** (`D-23`) המפרט בדיוק מאילו דרישות המועמד/ת פטור/ה |
...
> **אכיפת `Q-13` היא ברמת הבקשה, לא ברמת המועמד/ת** (הוכרע 2026-08-27).
> הייחודיות נבדקת על הצירוף `CandidateId` + `ProgrammeId` + סטטוס פעיל **בשורת הבקשה עצמה**,
> ולא כמגבלה על רשומת המועמד/ת. המשמעות המעשית: מועמד/ת יכול/ה להחזיק כמה בקשות פעילות במקביל —
> אחת לכל תוכנית — וזה בדיוק מה ש-`R-09` מתיר ("בקשה פעילה אחת לכל תוכנית, כמה תוכניות שתרצו").
> אכיפה ברמת המועמד/ת הייתה מגבילה לבקשה פעילה אחת בסך הכול, וסותרת את הכלל.
> האכיפה עצמה נעשית ב-`CEX-PortalStartApplication` לפני היצירה.
...
- `smkb_name` · `smkb_cex_ApplicationId` (`lookup`) · **`smkb_cex_CandidateId` (`lookup`)** ·
  ...
  `smkb_cex_SupersededById` (`lookup`) ·
  **`smkb_cex_Pdf` (`File`)** · `smkb_cex_VerificationCode` (`nvarchar(40)`, חובה)

> **`CandidateId` הוא דה-נורמליזציה מכוונת, לא קשר חסר** (הוכרע 2026-08-27).
> הקשר למועמד/ת **כבר קיים** דרך `Certificate → ApplicationId → CandidateId`, ובקשה מחייבת
> `CandidateId`, כך שכל אישור נפתר ממילא למועמד/ת אחד/ת בדיוק. העמודה הישירה נוספת לנוחות
> שאילתה — `S-P-10` ("האישורים שלי") ואימות מצד המדור — ולעמידות אם רשומת הבקשה תיארכב אי-פעם.
>
> **המחיר, ויש כזה:** שני מסלולים לאותה עובדה שיכולים לסתור זה את זה. Dataverse **אינו** מסנכרן
> ביניהם. לכן: **כותב יחיד בלבד** — `smkb_cex_CandidateId` נכתב פעם אחת, ב-`CEX-StaffFinaliseApplication`,
> מתוך `Application.CandidateId` באותה פעולה שיוצרת את האישור, ולעולם אינו מתעדכן אחר כך.
> אין תרחיש שבו מעדכנים אותו בנפרד; אם התגלה פער — הבקשה היא מקור האמת.
- **קוד האימות מודפס על האישור** בתסדיר **`Cert-XXXXXXXX`** (`O-5`, הוכרע 2026-08-27).
  המדור לענייני סטודנטים נדרש לאמת שאישור שהוצג לו אמיתי, ולכן האישור חייב לשאת מזהה שניתן לבדיקה
- **הערך אקראי, לא רץ.** מספר סידורי היה ניתן למניה — מי שמחזיק `Cert-A1B2C3D4` יכול לנחש את הבא.
...
- **התסדיר הוא הפתרון למתח בין `§12.2` ל-`§2.2`:** `§2.2` אוסר להציג לסטודנט/ית מזהה פנימי,
  ו-`§12.2` מחייב שהאישור יהיה ניתן לאימות. `Cert-XXXXXXXX` **אינו** המזהה הפנימי של אף רשומה —
...
> **האישור הוא הערכה מייעצת, לא מסמך מחייב** (`D-23`, 2026-08-30).
> אין לו תוקף משפטי והוא אינו מחייב את המדור לענייני סטודנטים — ההחלטה הסופית נשארת אצל הגורם
> האקדמי המוסמך בעת ההרשמה ללימודים. האישור **מתעד** שיפוט מקצועי, אינו מחליף אותו.
>
> **מימוש:** התבנית נושאת פקד תוכן `AdvisoryDisclaimer` בנוסח
> `תעודה זו הינה הערכה בלבד ועשויה להשתנות בעת הגשת בקשה ללימודים עצמם`,
> מוצג **בולט** — ממוסגר, מוצלל, מודגש, 14pt, ממורכז — מתחת לטבלת הפטורים ולא באותיות קטנות.
> `M-03` אינה מבטיחה פטור בפועל, ו-`S-P-08`/`S-P-10` מלווים את ההורדה בהסבר על האופי המייעץ.
>
> **מתח מוכר מול `PRODUCT_SPEC.md`:** `§12.2` ו-`§1.4` מתארים את האישור כנושא את מלוא התוצאה,
> ו-`§1.4` גם קובע שאין מערכת במורד הזרם שמקבלת את ההחלטה. `PRODUCT_SPEC.md` **נותר כפי שהוא**
> בהחלטה מכוונת; המתח מתועד ב-`D-23` והשאלה מי כן הופך פטור לתקף פתוחה כ-`O-7`.
...
| `CEX-PortalUploadEvidence` | העלאה + ולידציה [`FLOW_SNIPPETS.md` `§16`](SMKB%20-%20Course%20Exemption%20-%20Cloud%20Flows/FLOW_SNIPPETS.md) — **קובץ אחד לכל קריאה** (`D-24`) | `Evidence` | — |
...
| ~~`CEX-StaffGetEvidenceUrl`~~ | ~~כתובת קצרת-מועד לצפייה במסמך~~ | **מבוטל על ידי `D-24`** — הצוות קורא ב-Web API ישיר. ראו `X-3` |
| `CEX-StaffDecideSuggestion` | אישור/סירוב + נימוק | כותב `DecisionHistory` |
| `CEX-StaffUndoDecision` | ביטול החלטה בזמן שהתיק פתוח | `R-06` |
| `CEX-StaffFinaliseApplication` | **מפיק אישור ואז שולח `M-03`** | סדר מחייב. האישור מייעץ (`D-23`) |
...
### `X-3` — קבצים: התעבורה מפוצלת לפי משטח (`D-24`)

**העלאה נתמכת מקצה לקצה.** `buildNamedFilePayload` → base64 → flow → ולידציה
[`FLOW_SNIPPETS.md` `§16`](SMKB%20-%20Course%20Exemption%20-%20Cloud%20Flows/FLOW_SNIPPETS.md).
...
> **⚠️ ההחלטה הקודמת בוטלה על ידי `D-24`.** נכתב כאן: *"לא להזרים קבצים דרך flow — ה-flow מחזיר
> כתובת הורדה קצרת-מועד והדפדפן מושך ישירות."* **למנגנון הזה אין מימוש ב-Dataverse עבור
> `File Column`:** אין `SAS` ואין כתובת אנונימית קצרת-מועד, ו-`GET .../<column>/$value` דורש
> טוקן `Bearer`. אומת ב-[`SPIKE-FINDINGS.md`](SPIKE-FINDINGS.md). ה-spike שנדרש כאן בוצע, והוא
> גם שהפריך את ההנחה.

**ההחלטה המחליפה — `D-24`, מפוצלת לפי משטח:**

| מקום | משטח | מטען | תעבורה |
|---|---|---|---|
| `S-P-10` הורדת האישור | פורטל, אנונימי | אישור אחד, קטן | **זרימת base64 דרך flow** |
| `S-R-02` הצגת ראיות | צוות, מאומת | עד 5 × 4MB | **Web API ישיר** (`GET`) |

הפיצול נובע ממודל הזהות, לא מהעדפה: משתמשי הפורטל **אנונימיים כלפי Power Pages** ולכן אין להם
אבטחה ברמת השורה, בעוד משתמשי ה-Code App מאומתים ומקבלים אותה מהפלטפורמה. הנימוק המלא ב-`D-24`.

**זה נוגע בשני מקומות**, ו-`S-R-02` **חמור יותר** — הוא מגדיר את מיקום הראיות כ"דרישה קשיחה, לא
העדפת פריסה": החלטה שמתקבלת בלי גיליון הציונים על המסך היא ניחוש. הוא גם המשטח שמקבל את הפתרון
הטוב יותר.

**העלאה בצד הפורטל: קובץ אחד לכל קריאה** (~5.5MB), לא אצווה. כך תקרת ה-base64 **נעקפת בלי
להיבדק**.
...
### `X-5` — תיעוד גישה לראיות ברמת האפליקציה (`D-11` תחת `D-24`)

**לתכנן במהלך כתיבת ה-flows בשלב 7 — לא אחר כך.**

בצד הפורטל כל קריאת ראיה רצה תחת **חשבון השירות המשותף** (`msdyn_Dataverse`). לכן יומן הביקורת
של Dataverse רושם "חשבון השירות קרא" ו**לעולם אינו יודע מי קרא מה**. עבור מערכת המחזיקה תעודות
זהות וגיליונות ציונים, "מי ניגש לרשומה הזו" היא שאלה שצפוי שתישאל.

**הנדרש:** כל קריאת ראיה נרשמת בשכבת האפליקציה עם הגורם האמיתי — בדומה ל-`DecisionHistory`.

**מדוע דווקא עכשיו:** אם זה לא נבנה בזמן כתיבת ה-flows, הוא לא ייבנה — והפער יתגלה בביקורת,
כשכבר אין יומן לשחזר ממנו. צד הצוות (`D-24`, Web API ישיר) מקבל ייחוס נכון מהפלטפורמה ואינו
זקוק לתוספת הזו; זהו **פער של צד הפורטל בלבד**.

### `X-6` — ⚠️ עקיפה זמנית ב-CI שחייבת לחזור לקדמותה בסוף שלב 7

**מה נעשה (2026-08-30):** לצעד `flow-lint` ב-`.github/workflows/ci.yml` נוסף `--pre-commit`.

**מה להחזיר:** להסיר את `--pre-commit` מאותה שורה. זה הכול. השורה מסומנת בקובץ ב-
`TEMPORARY (2026-08-30) - REVERT WHEN PHASE 7 AUTHORING COMPLETES`.

**מדוע נעשה:** שלב 5 הפעיל את `activate.powerAutomateFlows`, אבל שלב 7 עדיין לא החליף את
ה-flows לדוגמה של ה-starter. לכן `Workflows/*.json` ו-`Other/*.xml` עדיין נושאים `smkb_sol_`,
`[REPLACE` ו-GUID-ים של תבנית — **במכוון** — ושתי חוקות זמן-הפריסה
(`no-placeholders`, `xml-no-placeholders`) נכשלות עליהם **בצדק**. CI היה אדום **עשר ריצות
רצופות**, וההערה ב-`ci.yml` חזתה את זה מראש.

**מדוע זה מוצדק ולא הסתרה:** job אדום קבוע גרוע מ-job מצומצם — הוא אילף אותנו להפסיק לקרוא
את CI, ולכן כשלים אמיתיים באותו חלון (`Config drift`, `Install` ב-Power App) עברו בלי שהבחינו
בהם. `--pre-commit` מדלג **רק** על שתי החוקות האלה; כל השאר רצות. **שער ה-placeholders עדיין
נאכף במקום שבו הוא מונע נזק** — `deploy.ps1` של כל starter ו-`/pre-deploy-verify`. שום דבר
אינו יכול להיפרס עם placeholders.

**ההיקף רחב מ-Cloud Flows:** חוקות ה-XML הן ברמת הפתרון כולו, ולכן גם `Dataverse Tables`
ו-`Environmental Variables` נושאים את אותם placeholders עד ששלב 7 ימלא אותם (17 שגיאות בסך
הכול, לא 9).

**התנאי להחזרה:** כשכל ה-flows, הטבלאות ומשתני הסביבה האמיתיים כתובים ואין יותר `smkb_sol_`
בשום starter מופעל. אם ההחזרה תישכח, השער החסר יתגלה בדיוק ברגע שבו הוא נחוץ.

### `X-7` — נקודת הכניסה שמפעילה את plugin הניקוד (`D-04`)

**להכריע לפני כתיבת ה-plugin, ואחרי טיוטת ה-Cloud Flows** — כדי שנדע כיצד
`CEX-PortalSubmitApplication` ו-`CEX-AdminPreviewRuleSet` קוראים אליו בפועל.

**הפער:** `CEX-PortalSubmitApplication` צעד 5 אומר רק "הפעלת ניתוח ההתאמות (`D-04`)".
`X-4` קובע שה-plugin נרשם דרך Plug-in Registration Tool או כרכיב פתרון, ו**אינו אומר מה מפעיל
אותו**. אין במפרטים אזכור ל-`Custom API`, ל-`unbound action` או ל-`SDK step`.

| אפשרות | יתרון | חיסרון |
|---|---|---|
| **`Custom API`** (unbound action) | הפעלה מפורשת מ-flow; ניתן לקרוא גם מ-`CEX-AdminPreviewRuleSet` להרצה יבשה; ניתן לבדיקה בנפרד | רכיב נוסף לתחזק ולפרוס |
| **`SDK message-processing step`** על `Application` | אין משטח API; רץ אוטומטית | הפעלה מכוונת (תצוגה מקדימה, ניקוד מחדש) קשה יותר; פחות שקוף |

> **הבניין הקודם השתמש ב-`Custom API`** (`smkb_cex_ScoreApplication`, נמחק בניקוי הסביבה).
> זו אינדיקציה, לא הכרעה — הארכיטקטורה החדשה החליפה את שלוש ה-APIs האחרות ב-flows.
...
| ~~`O-5`~~ | האם `VerificationCode` על האישור סותר את איסור המזהים? | — | ✅ **הוכרע 2026-08-27: כן להציג**, בתסדיר `Cert-XXXXXXXX`. המדור נדרש לאמת אישורים |
| `O-7` | אם האישור אינו מחייב — מה כן הופך פטור לתקף? | `R-01`, `M-03`, `S-P-10` | פתוח — נוצר על ידי `D-23`. מוכרע על ידי המדור האקדמי |
